...
import { emojiPuzzles } from '../data/emoji_puzzles';
...
export interface InternalGameState {
  ...
  totalRounds: number;
  isHardMode: boolean;
  revealedIndices: number[]; // Menyimpan indeks huruf yang terbuka secara acak
...
const prepareHangmanWord = (name: string): LetterObject[][] => {
  ...
  switch (action.type) {
    case 'START_GAME': {
      ...
      if (gameStyle === GameStyle.Classic && firstRoundData) {
        const wordToProcess = firstRoundData.country?.name || firstRoundData.triviaQuestion?.answer || firstRoundData.city?.name || firstRoundData.word || firstRoundData.stadium?.name || firstRoundData.emojiPuzzle?.answer || '';
        const scrambled = prepareHangmanWord(wordToProcess);
        const flattened = scrambled.flat();
        const initialRevealed = [];
        if (!hardModeActive && flattened.length > 0) {
            initialRevealed.push(Math.floor(Math.random() * flattened.length));
        }
        ...
        return {
          ...
          revealedIndices: initialRevealed,
    ...
    case 'NEXT_ROUND': {
      ...
      const { gameMode, nextCountry, nextLetter, nextCategory, availableAnswersCount, nextWord, nextWordCategory, nextTriviaQuestion, nextCity, nextStadium, nextEmojiPuzzle } = action.payload;
      const wordToProcess = nextCountry?.name || nextTriviaQuestion?.answer || nextCity?.name || nextWord || nextStadium?.name || nextEmojiPuzzle?.answer || '';
      
      const scrambled = prepareHangmanWord(wordToProcess);
      const flattened = scrambled.flat();
      const initialRevealed = [];
      if (!state.isHardMode && flattened.length > 0) {
          initialRevealed.push(Math.floor(Math.random() * flattened.length));
      }
    ...
    case 'PROCESS_COMMENT': {
        ...
        if (state.gameMode === GameMode.ABC5Dasar && state.currentLetter && state.currentCategory) {
            ...
            switch (state.gameMode) {
                ...
                case GameMode.GuessTheEmoji: expectedAnswer = state.currentEmojiPuzzle?.answer || ''; break;
            }
    ...
    case 'PREPARE_NEXT_MATCH': {
        ...
        if (!matchToStart || !matchToStart.player1 || !matchToStart.player2) return state;
        return {
            ...state,
    ...
    case 'SET_READY_TO_PLAY': return { ...state, gameState: GameState.KnockoutReadyToPlay };
    case 'RETURN_TO_BRACKET': return { ...state, gameState: GameState.KnockoutReadyToPlay };
    case 'REDRAW_BRACKET': {
        if (!state.knockoutPlayers || state.knockoutPlayers.length < 2) return state;
    ...
    case 'SET_KNOCKOUT_COUNTRY': {
        ...
        const scrambled = prepareHangmanWord(country.name);
        const flattened = scrambled.flat();
        return {
            ...state,
    ...
    case 'SET_KNOCKOUT_TRIVIA': {
        ...
        const scrambled = prepareHangmanWord(question.answer);
        const flattened = scrambled.flat();
        return {
            ...state,
    ...
    case 'SET_KNOCKOUT_ZONA_BOLA': {
        ...
        const scrambled = prepareHangmanWord(name);
        const flattened = scrambled.flat();
        return {
            ...state,
    ...
    case 'SET_KNOCKOUT_FOOTBALL_TRIVIA': {
        const question = action.payload.question;
        const scrambled = prepareHangmanWord(question.answer);
        const flattened = scrambled.flat();
        return {
     ...
     case 'SET_KNOCKOUT_GUESS_THE_FRUIT': {
        ...
        const scrambled = prepareHangmanWord(fruit);
        const flattened = scrambled.flat();
        return {
            ...state,
    ...
    case 'SET_KNOCKOUT_GUESS_THE_ANIMAL': {
        ...
        const scrambled = prepareHangmanWord(animal);
        const flattened = scrambled.flat();
        return {
            ...state,
            gameMode: GameMode.GuessTheWord,
    ...
    case 'SET_KNOCKOUT_ZONA_FILM': {
        ...
        const scrambled = prepareHangmanWord(movie);
        const flattened = scrambled.flat();
        return {
            ...state,
    ...
    case 'SET_KNOCKOUT_GUESS_THE_EMOJI': {
        const puzzle = action.payload.puzzle;
        const scrambled = prepareHangmanWord(puzzle.answer);
        const flattened = scrambled.flat();
        return {
            ...state,
    ...
    case 'KNOCKOUT_QUESTION_TIMEOUT': return { ...state, isRoundActive: false };
    case 'SKIP_KNOCKOUT_MATCH': return {
            ...state,
            isRoundActive: false,
            gameState: GameState.KnockoutReadyToPlay,
            knockoutMatchPoints: { player1: 0, player2: 0 },
    ...
    case 'FINISH_GAME': return { ...state, gameState: GameState.Finished };
    case 'RETURN_TO_MODE_SELECTION': return {
            ...createInitialState(),
            leaderboard: state.leaderboard,
            gameState: GameState.ModeSelection,
        };
    case 'RESET_GLOBAL_LEADERBOARD': return { ...state, leaderboard: [] };
...
export const useGameLogic = () => {
  ...
  const getNextEmojiPuzzle = useCallback(() => getNextUniqueItem(emojiDeck, emojiPuzzles, 'emojiPuzzles', p => p.emoji), [getNextUniqueItem]);
  ...
  const getNextZonaBola = useCallback(() => {
      ...
      if (choice === 0) return { type: 'Pemain Bola' as const, data: getNextFootballPlayer() };
      else if (choice === 1) return { type: 'Klub Bola' as const, data: getNextFootballClub() };
      else return { type: 'Stadion Bola' as const, data: getNextFootballStadium() };
  }, [getNextFootballPlayer, getNextFootballClub, getNextFootballStadium]);
  
  const startGame = useCallback((gameStyle: GameStyle, maxWinners: number, options?: { knockoutCategory?: KnockoutCategory, classicCategories?: GameMode[], useImportedOnly?: boolean, totalRounds?: number, isHardMode?: boolean }) => {
    ...
    if (gameStyle === GameStyle.Classic) {
        ...
        if (classicCategories.length > 0) {
          for (let i = 0; i < roundsToPlay; i++) roundDeck.push(classicCategories[i % classicCategories.length]);
        }
        const classicRoundDeck = shuffleArray(roundDeck);
        const firstRoundMode = classicRoundDeck.length > 0 ? classicRoundDeck[0] : GameMode.GuessTheFlag;
        ...
        } else if (firstRoundMode === GameMode.Trivia) firstRoundData.triviaQuestion = getNextTrivia();
        else if (firstRoundMode === GameMode.GuessTheCity) firstRoundData.city = getNextCity();
        else if (firstRoundMode === GameMode.ZonaBola) {
            const { type, data } = getNextZonaBola();
            firstRoundData.wordCategory = type;
            if (type === 'Stadion Bola') firstRoundData.stadium = data as FootballStadium;
            else firstRoundData.word = data as string;
        } else if (firstRoundMode === GameMode.FootballTrivia) {
        ...
        } else if (firstRoundMode === GameMode.KpopTrivia) firstRoundData.triviaQuestion = getNextKpopTrivia();
        else if (firstRoundMode === GameMode.ZonaFilm) {
            firstRoundData.word = getNextMovie();
            firstRoundData.wordCategory = 'Film';
        } else if (firstRoundMode === GameMode.BikinEmosi) firstRoundData.triviaQuestion = getNextBikinEmosi();
        else if (firstRoundMode === GameMode.GuessTheEmoji) firstRoundData.emojiPuzzle = getNextEmojiPuzzle();
  ...
  }, [prepareNewDecks, getNextCountry, getNextAbcCombo, getNextTrivia, getNextCity, getNextZonaBola, getNextFootballTrivia, getNextFruit, getNextAnimal, getNextKpopTrivia, getNextMovie, getNextBikinEmosi, getNextEmojiPuzzle]);
  ...
  const nextRound = useCallback(() => {
    ...
    const nextGameMode = state.classicRoundDeck[state.round];
    const payload: Partial<GameActionPayloads['NEXT_ROUND']> = { gameMode: nextGameMode };
    
    if (nextGameMode === GameMode.GuessTheFlag) payload.nextCountry = getNextCountry();
    ...
    } else if (nextGameMode === GameMode.Trivia) payload.nextTriviaQuestion = getNextTrivia();
    else if (nextGameMode === GameMode.GuessTheCity) payload.nextCity = getNextCity();
    else if (nextGameMode === GameMode.ZonaBola) {
        const { type, data } = getNextZonaBola();
        payload.nextWordCategory = type;
        if (type === 'Stadion Bola') payload.nextStadium = data as FootballStadium;
        else payload.nextWord = data as string;
    } else if (nextGameMode === GameMode.FootballTrivia) {
    ...
    } else if (nextGameMode === GameMode.KpopTrivia) payload.nextTriviaQuestion = getNextKpopTrivia();
    else if (nextGameMode === GameMode.ZonaFilm) {
        payload.nextWord = getNextMovie();
        payload.nextWordCategory = 'Film';
    } else if (nextGameMode === GameMode.BikinEmosi) payload.nextTriviaQuestion = getNextBikinEmosi();
    else if (nextGameMode === GameMode.GuessTheEmoji) payload.nextEmojiPuzzle = getNextEmojiPuzzle();
    dispatch({ type: 'NEXT_ROUND', payload: payload as GameActionPayloads['NEXT_ROUND'] });
  }, [state.round, state.totalRounds, state.gameStyle, state.classicRoundDeck, getNextCountry, getNextAbcCombo, getNextTrivia, getNextCity, getNextZonaBola, getNextFootballTrivia, getNextFruit, getNextAnimal, getNextKpopTrivia, getNextMovie, getNextBikinEmosi, getNextEmojiPuzzle]);

  const finishWinnerDisplay = useCallback(() => {
      dispatch({ type: 'HIDE_WINNER_MODAL' });
      if (state.round < state.totalRounds) dispatch({ type: 'START_COUNTDOWN' });
      else dispatch({ type: 'NEXT_ROUND', payload: {} as GameActionPayloads['NEXT_ROUND'] });
  ...
  const skipRound = useCallback(() => {
    if (state.isRoundActive || state.gameState === GameState.KnockoutPlaying) {
        if (state.gameStyle === GameStyle.Classic) dispatch({ type: 'END_ROUND' });
        else if (state.gameStyle === GameStyle.Knockout) dispatch({ type: 'SKIP_KNOCKOUT_MATCH' });
    }
  ...
  const registerPlayer = useCallback((player: KnockoutPlayer) => {
    if(state.gameState === GameState.KnockoutRegistration) dispatch({ type: 'REGISTER_PLAYER', payload: player });
  }, [state.gameState]);
  ...
  const endRegistrationAndDrawBracket = useCallback(() => {
    if(state.gameState === GameState.KnockoutRegistration) dispatch({ type: 'END_REGISTRATION_AND_DRAW_BRACKET' });
  }, [state.gameState]);
  const prepareNextMatch = useCallback((payload: { roundIndex: number; matchIndex: number }) => {
      if(state.gameState === GameState.KnockoutReadyToPlay) dispatch({ type: 'PREPARE_NEXT_MATCH', payload });
  }, [state.gameState]);
  ...
  useEffect(() => {
    ...
    if (state.isRoundActive && state.roundTimer > 0) {
      ...
      if (state.gameStyle === GameStyle.Classic && state.isRoundActive) dispatch({ type: 'END_ROUND' });
      else if (state.gameStyle === GameStyle.Knockout && state.isRoundActive) dispatch({ type: 'KNOCKOUT_QUESTION_TIMEOUT' });
    }
    return () => clearInterval(timerId);
  }, [state.isRoundActive, state.roundTimer, state.gameState, state.gameStyle]);

  // LOGIKA AUTO-REVEAL UNTUK HANGMAN (Satu slot acak setiap 15 detik)
  useEffect(() => {
    let revealTimer: number;
    if (state.isRoundActive && !state.isHardMode) {
        revealTimer = window.setInterval(() => {
            dispatch({ type: 'REVEAL_CLUE' });
        }, 15000); 
    }
    return () => clearInterval(revealTimer);
  }, [state.isRoundActive, state.isHardMode]);
  ...
  useEffect(() => {
    ...
    if (state.countdownValue && state.countdownValue > 0) {
      timerId = window.setInterval(() => dispatch({ type: 'TICK_COUNTDOWN' }), 1000);
        ...
        if(state.gameState === GameState.KnockoutPrepareMatch) dispatch({ type: 'START_MATCH' });
        else if (state.gameStyle === GameStyle.Classic) nextRound();
    }
    return () => clearInterval(timerId);
  ...
  useEffect(() => {
    if(state.gameState === GameState.KnockoutDrawing) {
        const timer = setTimeout(() => dispatch({ type: 'SET_READY_TO_PLAY' }), 2000);
  ...
  useEffect(() => {
    if (state.gameState === GameState.KnockoutPlaying && !state.isRoundActive && !state.currentCountry && !state.currentTriviaQuestion && !state.currentWord && !state.currentStadium && !state.currentEmojiPuzzle) {
        ...
        } else if (state.knockoutCategory === 'FootballTrivia') {
            const question = getNextFootballTrivia();
            dispatch({ type: 'SET_KNOCKOUT_FOOTBALL_TRIVIA', payload: { question } });
        ...
        } else if (state.knockoutCategory === 'GuessTheEmoji') {
            const puzzle = getNextEmojiPuzzle();
            dispatch({ type: 'SET_KNOCKOUT_GUESS_THE_EMOJI', payload: { puzzle } });
        }
    }
  }, [state.gameState, state.isRoundActive, state.knockoutCategory, getNextCountry, getNextTrivia, getNextZonaBola, getNextFootballTrivia, getNextFruit, getNextAnimal, getNextKpopTrivia, getNextMovie, getNextEmojiPuzzle]);

  useEffect(() => {
    if (state.gameState === GameState.KnockoutPlaying && !state.isRoundActive && (state.currentCountry || state.currentTriviaQuestion || state.currentCity || state.currentWord || state.currentStadium || state.currentEmojiPuzzle)) {
      const timeoutId = setTimeout(() => {
        ...
        if (match && (player1 >= KNOCKOUT_TARGET_SCORE || player2 >= KNOCKOUT_TARGET_SCORE)) {
          ...
          if(state.knockoutCategory === 'GuessTheCountry') dispatch({ type: 'SET_KNOCKOUT_COUNTRY', payload: { country: getNextCountry() } });
          else if (state.knockoutCategory === 'Trivia') dispatch({ type: 'SET_KNOCKOUT_TRIVIA', payload: { question: getNextTrivia() } });
          else if (state.knockoutCategory === 'ZonaBola') dispatch({ type: 'SET_KNOCKOUT_ZONA_BOLA', payload: getNextZonaBola() });
          else if (state.knockoutCategory === 'FootballTrivia') dispatch({ type: 'SET_KNOCKOUT_FOOTBALL_TRIVIA', payload: { question: getNextFootballTrivia() } });
          else if (state.knockoutCategory === 'GuessTheFruit') dispatch({ type: 'SET_KNOCKOUT_GUESS_THE_FRUIT', payload: { fruit: getNextFruit() } });
          else if (state.knockoutCategory === 'GuessTheAnimal') dispatch({ type: 'SET_KNOCKOUT_GUESS_THE_ANIMAL', payload: { animal: getNextAnimal() } });
          else if (state.knockoutCategory === 'KpopTrivia') dispatch({ type: 'SET_KNOCKOUT_KPOP_TRIVIA', payload: { question: getNextKpopTrivia() } });
          else if (state.knockoutCategory === 'ZonaFilm') dispatch({ type: 'SET_KNOCKOUT_ZONA_FILM', payload: { movie: getNextMovie() } });
          else if (state.knockoutCategory === 'GuessTheEmoji') dispatch({ type: 'SET_KNOCKOUT_GUESS_THE_EMOJI', payload: { puzzle: getNextEmojiPuzzle() } });
        }
      }, 3000);
      return () => clearTimeout(timeoutId);
    }
  }, [state.isRoundActive, state.gameState, state.knockoutMatchPoints, state.knockoutCategory, getCurrentKnockoutMatch, getNextCountry, getNextTrivia, getNextZonaBola, getNextFootballTrivia, getNextFruit, getNextAnimal, getNextKpopTrivia, getNextMovie, getNextEmojiPuzzle]);
  ...
  useEffect(() => {
    if (state.gameStyle === GameStyle.Classic && state.isRoundActive) {
        const currentMaxWinners = state.gameMode === GameMode.ABC5Dasar && state.availableAnswersCount != null 
              ? Math.min(state.maxWinners, state.availableAnswersCount) 
              : state.maxWinners;
        if (state.roundWinners.length >= currentMaxWinners) dispatch({ type: 'END_ROUND' });
    }
  ...
  useEffect(() => {
    ...
    if (state.gameState === GameState.Champion) {
      timeoutId = window.setTimeout(() => {
        if (state.gameStyle === GameStyle.Classic) dispatch({ type: 'FINISH_GAME' });
        else dispatch({ type: 'RETURN_TO_BRACKET' });
  ...
  const getCurrentAnswer = () => {
    switch (state.gameMode) {
      ...
      case GameMode.GuessTheFruit:
      case GameMode.GuessTheAnimal:
      case GameMode.ZonaFilm: return state.currentWord || '';
      case GameMode.GuessTheCity: return state.currentCity?.name || '';
      case GameMode.ABC5Dasar: return `(Jawaban Kategori ${state.currentCategory} diawali huruf ${state.currentLetter})`;
      case GameMode.Trivia:
      case GameMode.KpopTrivia:
      case GameMode.BikinEmosi:
      case GameMode.FootballTrivia: return state.currentTriviaQuestion?.answer || '';
      case GameMode.ZonaBola: return state.currentWord || state.currentStadium?.name || '';
      case GameMode.GuessTheEmoji: return state.currentEmojiPuzzle?.answer || '';
      default: return 'Tidak ada soal aktif';
    }
  };
